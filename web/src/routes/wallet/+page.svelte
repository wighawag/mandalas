<script lang="ts">
  ...
  let isWalletOwner = $derived(
    currentAddress && walletAddress && currentAddress.toLowerCase() === walletAddress.toLowerCase(),
  );
...
<div class="w-full">
  ...
  {#if !connected}
    <div class="w-full h-full mx-auto text-center flex-col text-black dark:text-white">
      <p class="mt-4 text-xs sm:text-base font-black text-yellow-400">Please Connect to your wallet see the tokens</p>
      <button
  ...
  {:else if !walletAddress || walletAddress === ''}
    <div class="w-full h-full mx-auto text-center flex-col text-black dark:text-white">
      <p class="mt-4 text-xs sm:text-base font-black text-yellow-400">Please Connect to your wallet see your tokens</p>
      <button
        class="m-2 text-xs md:text-base font-black text-yellow-400 border border-yellow-500 p-1"
        onclick={() => connection.connect({type: 'wallet'})}
      >
  ...
  {#if $nfts.state === 'Ready'}
    {#if $nfts.tokens.length > 0}
      <div class="w-full h-full mx-auto flex flex-col items-center justify-center text-black dark:text-white">
        <p class="p-6">
          {#if isWalletOwner}
            Here are your Mandalas. You can burn them to get 95% of the current price. Each time a mandala is burnt, the
            price decrease. Note that once burnt that same Mandala cannot be re-created.
