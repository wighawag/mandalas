...
export type CoreServices = {
	...
	/**
	 * Sends from the authenticated account. The mint is the only thing this app
	 * sends, and the account pays for it, so there is one executor and the
	 * purchase flow names it.
	 */
	accountExecutor: Context['accountExecutor'];
	/** Gas held by the account that pays: what the mint is checked against. */
	accountBalance: Context['accountBalance'];
	balanceCheck: Context['balanceCheck'];
	/** Where a failed mint's full error text goes, for the details dialog. */
...
export type AppContext = {
	...
	/** Locally-generated candidate Mandalas for the browse grid. */
	randomTokens: Context['randomTokens'];
	/** Mandalas owned by an address (undefined = the connected account). */
	nftsOf: Context['nftsOf'];
	/** The mint dialog's state machine (price quote -> confirm -> submit). */
	purchaseFlow: Context['purchaseFlow'];
	/**
...
export function createCoreContext<App extends AppContext>(params: {
	...
	const {finality, txObserverProcessInterval, batchSize} =
	...
	// HOISTED ABOVE THE APP HALF, and upstream builds them below it. The reason is
	// this app's purchase flow: it quotes a rising price, checks the buyer can
	// cover it and reports what went wrong, so it needs `accountBalance` and
	// `balanceCheck` at CONSTRUCTION, and the app half is built before them
	// upstream. Only these three moved; everything that genuinely depends on the
	// app's `onchainState` (the refresh connector, `rpcHealth`, `refreshChainData`)
	// stays below, where upstream has it. The alternative was handing the app a
	// placeholder store to fill in later, which is a mutable hole in the one file
	// that must stay easy to merge.
