...
export class Blockie {
	...
	draw(
		...
		for (let i = 0; i < this.imageData.length; i++) {
			...
			ctx.fillStyle = hslToStyle(
				this.imageData[i] == 1 ? this.color : this.spotcolor,
			);
	...
	createColor(): {h: number; s: number; l: number} {
		...
		const l =
			Math.floor(
				1000 * (this.rand() + this.rand() + this.rand() + this.rand()) * 25,
			) / 100000;
		return {h, s, l};
	}
