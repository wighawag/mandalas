...
export const CHROME = [
	...
	chromeBar(
		...
		{
			// MANDALAS' OWN GATE, not the template's `routeId !== '/'`. This banner is
			// relevant on pages that read onchain data. The standalone mandala
			// renderer does not (it is pure client-side generation from the id in the
			// hash), and neither does /about, so both are excluded while / is not.
			// Moved here from a `showRpcBanner` derived in `routes/+layout.svelte`,
			// which is the file the template churns hardest; the condition is
			// unchanged. A route id rather than a pathname: it is base-path
			// independent, so this still works under IPFS and relative deploys.
			when: ({routeId}) => routeId !== '/mandala' && routeId !== '/about',
		},
	),
