...
describe('the chrome list', () => {
	 ...
	 * intact.
	 *
	 * MANDALAS' GATE, NOT THE TEMPLATE'S, and the difference is the point. Upstream
	 * excludes `/` because its home page reads no onchain data. Here the excluded
	 * routes are the standalone renderer (pure client-side generation from the id
	 * in the hash) and `/about`, while `/` is a route where the bar SHOULD show.
	 * The condition is the one that used to be a `showRpcBanner` derived in
	 * `routes/+layout.svelte`, carried over unchanged.
	 */
	it('keeps the RPC bar off the renderer and /about, and nowhere else', () => {
		...
		for (const routeId of ['/mandala', '/about']) {
			expect(rpc!.when!({routeId}), `hidden on ${routeId}`).toBe(false);
		}
		for (const routeId of ['/', '/transactions', '/explorer', null]) {
