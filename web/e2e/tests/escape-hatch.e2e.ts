...
// carry (see the same note on layout-shell.e2e.ts). That fixture exists to clear
// localStorage so a previous test's auto-connect cannot leak in; Playwright
// already gives each test a fresh context, and nothing here reads the burner
// account list it also provides, because this suite connects the STALLING wallet
// by name.
 ...
 * here rather than adapted. The behaviour it covers is real in this app and was
 * left uncovered; this restores it. The code under test is the same
 ...
 * token or prior mint. See e2e/fixtures/stalling-wallet.ts for why that is
...
describe('Stopping waiting for the wallet', () => {
	...
	// load `/contracts` (which reads MandalaToken), connect, then send. That is the heaviest walk in the
	...
	test('survives the reconnect that used to erase the request', async ({
		...
		// The next test drives that, by connecting a wallet again; this comment used to
		...
		// different reason: it routes through `connect()`, which tears the wallet
		// down and rebuilds it. Rebuilding from a wallet-less state is the next
	...
	test('keeps announcing a request through a state with NO wallet', async ({
		...
		//
		// DRIVEN THROUGH `cancel()` HERE, where the template drives `connect()`.
		//
		// The template reaches a wallet-less resting state by opening the wallet
		// PICKER, which needs two wallets on offer. This app announces only the
		// stalling wallet, so @etherplay/connect connects to it directly and the
		// picker never appears; the inherited version asserted `WalletToChoose` and
		// got `WalletConnected`. `cancel()` is this app's shortest route to the same
		// place: it lands on Idle with `wallet: undefined`, which is a resting state
		// that carries no wallet, and that is the property under test.
		//
		// It is also the destructive path on purpose. `stopWaitingForWallet` refuses
		// to call `cancel()` while a transaction is outstanding precisely because it
		// throws away account data; driving it directly here asserts that even THAT
		// state keeps announcing what the user's wallet is still holding.
		...
		await page.evaluate(() => (globalThis as any).context.connection.cancel());
		...
		await expect
			...
			.toEqual({step: 'Idle', hasWallet: false});
		...
		// the list" - and neither can resting with no wallet. CONNECTING A WALLET
		// AGAIN is what constructs one from scratch, under a request the user's
		// wallet is still holding, which is exactly the shape that lost it.
		//
		...
		//
		// Driven with the wallet NAMED, which in the template is what the picker
		// row's `onclick` does and here is simply the only way: this app announces
		// one wallet and never opens a picker (see `cancel()` above), so there is no
		// row to click even in principle. Same call either way
		// (ConnectionFlow.svelte: `connect({type: 'wallet', name})`), reached past
		// the UI exactly as `cancel()` above is.
		await page.evaluate(
			(name) =>
