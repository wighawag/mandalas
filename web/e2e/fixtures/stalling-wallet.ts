...
export async function sendAndStall(
	...
	// and this suite was deleted here rather than adapted. `/contracts` is still
	// here, its Write tab calls the ACCOUNT executor, and that goes to the user's
	// wallet, which is the window this fixture exists to stand in. The mint and
	// burn flows are this app's real writes, but both need a token or a price
	// lookup first, and a fixture that has to mint before it can stall is a
	// fixture that fails for reasons that are not its subject.
	//
	// THE FUNCTION. `setApprovalForAll(address operator, bool approved)`, on
	// MandalaToken like every ERC721. Chosen because it CANNOT REVERT and needs
	// nothing set up first: no token, no mint, no price. That matters more than it
	// looks. A call the node can see will fail dies in GAS ESTIMATION, which
	// happens before `eth_sendTransaction`, so the wallet is never asked and there
	// is no held request to stop waiting for. `burn` and `transferFrom` both revert
	// here, because this account owns no token.
	//
	...
	// NOT `chooseStallingWallet(page)` HERE, and this app is why the difference
	// exists. Its sibling templates announce a burner alongside this fixture, so
	// two wallets are on offer and the flow always stops at a picker. Mandalas
	// announces only this one, so @etherplay/connect connects to it directly (see
	// the `wallets.length == 1` fallback in its `connect`) and NO picker is ever
	// shown. Waiting for one spent thirty seconds staring at an app that was
	// already connected and holding nothing back.
	//
	// So the choice is RACED inside the wait below rather than sequenced before
	// it, exactly as the sign-in click already is: click the picker if it appears,
	// and otherwise notice that the wallet is already holding the request.
...
async function waitUntilHolding(page: Page, timeout = 60_000): Promise<void> {
	...
	/** Which buttons have an attempt running, so they cannot pile up. */
	const inFlightClicks = new Set<string>();

	/**
	 * Click it if it is there, without making the poll wait for the answer.
	 *
	 * One attempt per button at a time, bounded, and never awaited: see the note
	 * on the sign-in click below for why each of those three matters.
	 */
	const startClick = async (name: string, button: Locator) => {
		if (inFlightClicks.has(name)) return;
		if (!(await button.isVisible().catch(() => false))) return;
		inFlightClicks.add(name);
		void button
			.click({timeout: SIGN_IN_CLICK_TIMEOUT})
			.catch(() => {})
			.finally(() => {
				inFlightClicks.delete(name);
			});
	};
	...
	while (Date.now() < deadline) {
		...
		// The picker, when this app shows one at all. Bounded and NOT awaited, for
		// the same two reasons as the sign-in click below: a click that loses its
		// race against the app moving on must fail rather than wait for the element
		// to come back, and it must not hold up the poll above while it does.
		await startClick('walletEntry', walletEntry);
		await startClick('stallingWallet', stallingWallet);
		...
		await startClick('signIn', signIn);
