...
```
npx degit wighawag/mandalas#nft <your-app-folder>
```
...
this will update name of the project (by default "mandalas") to be the name of the folder (See `set-name` command) and install the dependencies (`pnpm install`)
<br/><br/>

`pnpm set-name [<new name>]`

This will replace every instance of `mandalas` (or whatever name was set) to `new name` (if specified, otherwise it use the folder name)
